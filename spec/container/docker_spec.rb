...
describe LintTrap::Container::Docker do
  subject(:container){described_class.new('lintci/spin_cycle', '/local/path')}
  ...
  describe '#wrap' do
    it 'wraps the command passed in with a call to docker' do
      expect(container.wrap('ls')).to eq(
        ...
        '-v /local/path:/home/code --workdir=/home/code lintci/spin_cycle ls'
      )
    end
  end
  ...
  describe '#container_path' do
    it 'returns the absolute path of the file in the container' do
      expect(container.container_path('/local/path/bad.file')).to eq('/home/code/bad.file')
    end
  end

  describe '#local_path' do
    it 'returns the absolute path of the file outside the container' do
      expect(container.local_path('/home/code/bad.file')).to eq('/local/path/bad.file')
    end
  end
end
